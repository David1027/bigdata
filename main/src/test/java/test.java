...
public class test {
  ...
  public static void test() {
    RingBuffer<ValueEvent> buffer =
        RingBuffer.create(
            ProducerType.SINGLE, // 定义一个ringBuffer,也就是相当于一个队列
            ValueEvent.EVENT_FACTORY,
            1 << 2,
            new BlockingWaitStrategy());
    // 定义消费者,只要有生产出来的东西，该事件就会被触发,参数event 为被生产出来的东西
    // 几个workerHandler 表示有几个消费者
    WorkHandler<ValueEvent> workHandler =
        valueEvent -> {
          System.out.println(Thread.currentThread().getName() + " " + valueEvent.getValue());
        };
    ...
    workerPool.start(Executors.newFixedThreadPool(4)); // 用executor 来启动 workProcessor 线程
