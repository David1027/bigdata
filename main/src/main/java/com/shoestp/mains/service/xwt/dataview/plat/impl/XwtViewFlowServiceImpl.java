package com.shoestp.mains.service.xwt.dataview.plat.impl;
...
import com.shoestp.mains.dao.xwt.dataview.plat.dao.XwtViewFlowDAO;
import com.shoestp.mains.dao.xwt.dataview.plat.dao.XwtViewFlowPageDAO;
...
public class XwtViewFlowServiceImpl implements XwtViewFlowService {
  ...
  private Map<String, List<KeyValue>> getFlowSourceTypeTimeByHourMap(Date date) {
    ...
    flowTimeHourMap.put(Contants.DAY, keyValues);
   ...
   * @return long[]
   */
  private long[] getEveryHour(Date date, SourceTypeEnum source, String parameter) {
    long[] arr = new long[24];
    for (int i = 0; i < arr.length; i++) {
      if (Contants.VISITOR.equals(parameter)
          && getFlowSourceType(date, i, i + 1).get(source.toString()) != null) {
        arr[i] = getFlowSourceType(date, i, i + 1).get(source.toString()).getVisitorCount();
      } else if (Contants.VIEW.equals(parameter)
          && getFlowSourceType(date, i, i + 1).get(source.toString()) != null) {
        arr[i] = getFlowSourceType(date, i, i + 1).get(source.toString()).getViewCount();
      }
    }
    return arr;
  }
   ...
   * @return Map<String, List>
   */
  private Map<String, List<KeyValue>> getFlowSourceTypeTimeByHourMap(
      Date date, SourceTypeEnum sourceType) {
    List<KeyValue> keyValues = new ArrayList<>();
    keyValues.add(
        KeyValueViewUtil.getFlowKeyValue(
            Contants.VISITOR, getEveryHour(date, sourceType, Contants.VISITOR)));
    keyValues.add(
        KeyValueViewUtil.getFlowKeyValue(
            Contants.VIEW, getEveryHour(date, sourceType, Contants.VIEW)));
    Map<String, List<KeyValue>> flowTimeHourMap = new HashMap<>(16);
    flowTimeHourMap.put(Contants.DAY, keyValues);
    return flowTimeHourMap;
  }
   ...
   */
  @Override
  public Map<String, Map> getFlowSourceTypeTimeByHour(Date date, SourceTypeEnum sourceType) {
    Map<String, Map> flowTimeHourMap = new HashMap<>(16);
    flowTimeHourMap.put(Contants.ABSCISSA, DateTimeUtil.getHourAbscissa(1));
    flowTimeHourMap.put(Contants.SOURCE_TYPE, getFlowSourceTypeTimeByHourMap(date, sourceType));
    return flowTimeHourMap;
  }
  ...
  private Map<String, List<KeyValue>> getFlowSourceTypeTimeByDayMap(int num, Date date) {
    ...
    flowTimeDayMap.put(Contants.DAY, keyValues);
   ...
   * @return long[]
   */
  private long[] getEveryDay(int num, Date date, SourceTypeEnum source, String parameter) {
    long[] arr = new long[num];
    for (int i = 0; i < arr.length; i++) {
      if (Contants.VISITOR.equals(parameter)
          && getFlowSourceType(DateTimeUtil.getDayFromNum(date, num - i - 1), 0, 24)
   ...
   * @return Map<String, List>
   */
  private Map<String, List<KeyValue>> getFlowSourceTypeTimeByDayMap(
      int num, Date date, SourceTypeEnum sourceType) {
    List<KeyValue> keyValues = new ArrayList<>();
    keyValues.add(
        KeyValueViewUtil.getFlowKeyValue(
            Contants.VISITOR, getEveryDay(num, date, sourceType, Contants.VISITOR)));
    keyValues.add(
        KeyValueViewUtil.getFlowKeyValue(
            Contants.VIEW, getEveryDay(num, date, sourceType, Contants.VIEW)));
    Map<String, List<KeyValue>> flowTimeDayMap = new HashMap<>(16);
    flowTimeDayMap.put(Contants.DAY, keyValues);
    return flowTimeDayMap;
  }
   ...
   * 根据时间和类型获取流量概况(天)
   *
   * @author: lingjian @Date: 2020/1/7 10:41
   * @param num 天数
   * @param date 时间
   * @param sourceType 来源类型
   * @return Map<String, Map>
   */
  @Override
  public Map<String, Map> getFlowSourceTypeTimeByDay(
      int num, Date date, SourceTypeEnum sourceType) {
    Map<String, Map> flowTimeDayMap = new HashMap<>(16);
    flowTimeDayMap.put(Contants.ABSCISSA, DateTimeUtil.getDayAbscissa(num, date));
    flowTimeDayMap.put(Contants.SOURCE_TYPE, getFlowSourceTypeTimeByDayMap(num, date, sourceType));
    return flowTimeDayMap;
  }
      ...
      if (Contants.VISITOR.equals(parameter)
        ...
        arr[i] =
            CustomDoubleSerialize.setDouble(
                getFlowPageAnalysisByAccess(access, date, i, i + 1).getAverageStayTime() / 1000);
      }
    }
    return arr;
  }
      ...
      if (Contants.VISITOR.equals(parameter)
        ...
        arr[i] =
          ...
          && getFlowPageAnalysisByAccess(
              ...
              != null) {
        arr[i] =
                CustomDoubleSerialize.setDouble(getFlowPageAnalysisByAccess(
                    access, DateTimeUtil.getDayFromNum(date, num - i - 1), 0, 24)
                .getAverageStayTime() / 1000);
      }
    }
    return arr;
  }
  ...
  private XwtViewFlowPage getFlowPageByDate(Date date, Integer num) {
    ...
    today.setAverageStayTime(CustomDoubleSerialize.setDouble(today.getAverageStayTime() / 1000));
  ...
  private XwtViewFlowPage getFlowPageParameter(Date date) {
    ...
    // 平均停留时长
    result.setAverageStayTime(CustomDoubleSerialize.setDouble(result.getAverageStayTime() / 1000));
    return result;
  }
